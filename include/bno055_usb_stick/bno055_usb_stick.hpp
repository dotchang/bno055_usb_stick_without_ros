...
private:
  void start() {
    ...
    try {
	  ...
	  std::cerr << "start: " << error.what() << std::endl;
      restart();
      return;
    }
    ...
    if (mode_ == "ndof") {
	  ...
	  std::cerr << "Unknown mode \""
				<< mode_ << "\" was given. Will use the default mode \"ndof\" instead." << std::endl;
      for (const boost::uint8_t **command = Constants::toNDOFCommands(); *command; ++command) {
        commands_.push_back(*command);
      }
    }
  ...
  void startSendCommand() {
    if (commands_.empty()) {
	  std::cerr << "startSendCommand: No command in the queue" << std::endl;
      restart();
      return;
    }
  ...
  void handleSendCommand(const boost::system::error_code &error, const std::size_t bytes) {
    ...
    if (error) {
	  std::cerr << "handleSendCommand: " << error.message() << std::endl;
      restart();
      return;
    }
  ...
  void handleWaitResponse(const boost::system::error_code &error, const std::size_t bytes) {
    ...
    if (error) {
	  std::cerr << "handleWaitResponse: " << error.message() << std::endl;
      restart();
      return;
    }
  ...
  void handleWaitData(const boost::system::error_code &error, const std::size_t bytes) {
    ...
    if (error) {
	  std::cerr << "handleWaitData: " << error.message() << std::endl;
      restart();
      return;
    }
  ...
  void startWaitDeadline(void (BNO055USBStick::*handler)()) {
    timer_.expires_from_now(boost::posix_time::time_duration(0, 0, timeout_));
  ...
  void handleWaitDeadline(const boost::system::error_code &error,
    ...
    if (error == boost::asio::error::operation_aborted) {
	  // std::cerr << "handleWaitDeadline: the deadline disabled" << std::endl;
      return;
    } else if (error) {
	  std::cerr << "handleWaitDeadline: " << error.message() << std::endl;
      return;
    }
    ...
    try {
	  ...
	  std::cerr << "handleWaitDeadline: " << error_on_cancel.what() << std::endl;
    }
